...
export class StatsTracker {
  ...
  #siteEntry(s: StatsShape) {
    if (!s.bySite[this.#site]) s.bySite[this.#site] = { watchMs: 0, actions: 0 };
    return s.bySite[this.#site];
  }

  #mutate(fn: (s: StatsShape) => void) {
    if (!chrome.runtime?.id) return;
    try {
      chrome.storage.local.get([STATS_KEY], (r: { uvtStats?: StatsShape }) => {
        const s: StatsShape = r.uvtStats || { totalWatchMs: 0, totalActions: 0, bySite: {}, daily: {}, actionCounts: {} };
        fn(s);
      ...
      /* extension context invalidated (e.g. reload mid-session) — drop silently */
    }
  }
}
