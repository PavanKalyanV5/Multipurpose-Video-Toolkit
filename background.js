...
// VideoToolkit.js), so a fresh install otherwise does *nothing* until you
...
function resHint(url) {
  const m =
    url.match(/[\/_.-](144|240|360|480|540|720|1080|1440|2160)[pP]?[\/_.-]/) ||
    url.match(/(\d{3,4})[pP][\/_.-]/);
  return m ? m[1] + 'p' : '';
}
...
chrome.webRequest.onResponseStarted.addListener(
  (d) => {
    ...
    const isMedia =
      /video\/(mp4|webm|quicktime)/i.test(ct) ||
      /\.(mp4|webm|mov|m4v)(\?|$)/i.test(d.url);
    if (!isMedia) return;
    ...
    try {
      ...
      const u = new URL(url);
      ['bytestart', 'byteend', 'range', 'rn', 'rbuf'].forEach((p) =>
    ...
    // Feature #16: show badge dot so user knows something is downloadable.
    // The tab can close between the request completing and this running \u2014
    // catch rather than let it become an unhandled rejection in the worker.
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (msg && msg.type === 'uvt-inject-speed') {
    const tabId = sender.tab ? sender.tab.id : -1;
    if (tabId !== -1) {
      chrome.scripting.executeScript({
        target: { tabId: tabId, frameIds: [sender.frameId || 0] },
        world: 'MAIN',
        func: (rate) => {
          try {
  ...
  if (msg && msg.type === 'uvt-run-rule-js') {
    ...
    // own context (governed by *our* extension CSP, see manifest.json's
  ...
  if (msg && msg.type === 'uvt-report-state') {
    const tabId = sender.tab ? sender.tab.id : -1;
    const frameId = sender.frameId;
    if (tabId !== -1) {
      chrome.storage.local.set({
        [`uvt_active_${tabId}`]: { frameId, state: msg.state }
      });
  ...
  if (msg && msg.type === 'uvt-media-list') {
    const tabId = sender.tab ? sender.tab.id : -1;
    sendResponse({ items: mediaByTab[tabId] || [] });
    return;
  }
  if (msg && msg.type === 'uvt-download' && msg.url) {
