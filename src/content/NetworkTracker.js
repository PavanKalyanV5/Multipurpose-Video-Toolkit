...
export class NetworkTracker {
  ...
  start(onUpdate, intervalMs = 1000) {
  ...
  /**
   * Measure current video speed and device receiving speed.
   * @param {HTMLVideoElement} [video]
   * @returns {{ videoSpeed: string, deviceSpeed: string, videoBps: number, deviceMbps: number }}
   */
  measure(video = null) {
    const now = performance.now();
    const windowMs = 4000; // 4 second sliding window for chunk capture
    ...
    try {
      ...
      for (let i = entries.length - 1; i >= 0; i--) {
        ...
        if (endTime < cutoff) break; // entries are sorted chronologically
        ...
        if (isMedia && size > 0) {
          mediaBytes += size;
          // Calculate true fetch duration of the active chunk (excluding idle gap)
    ...
    if (video && video.buffered && video.buffered.length > 0) {
      try {
        // Find the specific buffer range containing current playback time (handles seeks)
        ...
        if (!video.paused && this.#lastBufferedEnd >= 0 && currentRangeEnd > this.#lastBufferedEnd) {
          ...
          if (deltaWallSec > 0 && deltaBufferSec < 60) { // filter out large seek jumps
    ...
    // If YouTube is in an idle period between buffer bursts while video plays smoothly from buffer,
    // hold the last active download speed rather than plunging aggressively to 0 B/s.
    if (hasActiveFetch) {
      ...
      // Gentle decay during YouTube idle buffer holds
      this.#videoSpeedBps = this.#videoSpeedBps * 0.92;
    } else {
      // Video is paused or buffer is empty — decay naturally to zero
    ...
    // Connection downlink speed from navigator.connection (if available)
    let deviceMbpsStr = '';
    let rawDownlinkMbps = 0;
    if (navigator.connection && typeof navigator.connection.downlink === 'number') {
      rawDownlinkMbps = navigator.connection.downlink;
