export class VideoScanner {
  #seen = new WeakSet();
  #speedApplied = new WeakSet();
  #toolkit; // reference to VideoToolkit for callbacks
  ...
  constructor(toolkit) { this.#toolkit = toolkit; }
  ...
  wire(video) {
    ...
    video.addEventListener('play', () => {
      ...
      if (tk.autoplayBlock && !video._uvtUserPlay) {
        video.pause();
        tk.ui.toast('Autoplay blocked — press play to continue.', 2000);
        return;
      }
      ...
      if (video === tk.ui.currentVideo) tk.reportState(video);
    ...
    video.addEventListener('click',  () => { video._uvtUserPlay = true; });
    ...
    video.addEventListener('ratechange', () => {
      if (video !== this.#toolkit.ui.currentVideo) return;
      this.#toolkit.ui.spdEl.textContent = video.playbackRate + 'x';
      this.#toolkit.ui.updatePill(this.#toolkit.booster, this.#toolkit.recorder);
      this.#toolkit.reportState(video);
    });
    video.addEventListener('mouseenter', () => {
      this.#toolkit.ui.cancelHide();
      this.#toolkit.ui.showPill(video);
    });
    video.addEventListener('mouseleave', () => this.#toolkit.ui.scheduleHide());
  }

  /** Scan a DOM node for video elements. */
  scan(node) {
    if (node.querySelectorAll) node.querySelectorAll('video').forEach((v) => this.wire(v));
  ...
  observe() {
    new MutationObserver((muts) => {
      let removed = false;
      for (const m of muts) {
        m.addedNodes.forEach((n) => n.nodeType === 1 && this.scan(n));
        m.removedNodes.forEach((n) => {
          if (n.nodeType !== 1) return;
          const videos = n.matches?.('video') ? [n] : Array.from(n.querySelectorAll?.('video') || []);
          if (videos.length) {
            removed = true;
            videos.forEach((v) => {
